# averaged_perceptron0
# the original averaged perceptron configured here for IOB tagging 
# takes pos-tags as input for features

from collections import defaultdict
import json, re, pickle, os, random, copy, codecs
from generalFunctions import *  # jsonify and pickling functions

class AveragedPerceptronIOB(object):
	...
	def _get_features(self, i, context, pos_context, prev, prev2):
	...
	def __init__(self, sentences, verbose=True):
		"""sentences is a list of triples of the form (words, pos_tags, IOB_tags).  
			words, pos_tags, IOB_tags are all lists.  This is the labeled-data input."""

		if verbose:
			print "AveragedPerceptronIOB initialization begun. \nInitiating tags..."

		# classes will be the possible tags
		self.tags=set()
		# populate tags set
		for words, pos_tags,iob_tags in sentences:
			for t in iob_tags:
		...
		for words, pos_tags, iob_tags  in sentences:
			...
			prev="n_START_"
			for i,word in enumerate(words):
				features=features.union(self._get_features(i,context,pos_context,prev,prev2))
				prev2=prev
				prev=iob_tags[i]
		for f in features:
			self.weights[f]={}
	...
	def train(self, save_path=None, num_it=5, verbose=False):
		...
		for iter in xrange(num_it):
			...
			for words, pos_tags, iob_tags  in self.sentences:
				...
				for i,word in enumerate(words):	
					...
					if not guess:
						features=self._get_features(i, context,pos_context, prev, prev2)
						guess=self.predict(features)
						self.update(features, iob_tags[i], guess) # we only update if we used the predictor 
					prev2=prev
					prev=guess
					#update the counts
					if guess==iob_tags[i]:
	...
	def tag_new_sentences(self, new_sentences, verbose=True):
		""" 
		new_sentences is a list of doubles (words, pos_tags), and this produces & returns the list of triples
		(words, pos_tags, new_tags) using the weights dictionary
		""" 
		if verbose:
		...
		for words, pos_tags in new_sentences:
			...
			for i,word in enumerate(words):	
				...
				prev2=prev
				prev=guess
			tagged_sentences.append((words, pos_tags, new_tags))
	...
	def test_accuracy(self, test_sentences,  verbose=True, save_path=None):
		...
		""" 
		new_sentences=[(words, pos_tags) for (words, pos_tags, iob_tags) in test_sentences]
...
## Testing
# sentences=[(["the","dog","ran"], ["D","N","V"], ["zz","zz",'yy']), (["a","cat","ran"], ["D","N","V"], ["yy","zz",'yy'])]
# new_sentences=[(["the","dog","ran"], ["D","N","V"]), (["a","horse","ran"], ["D","N","V"])]
# test_sentences=[(["the","dog", "and", "horse", "ate"], ["D","N", "C","N", "V"], ["zz","zz",'yy', "zz", "zz"])]

# A=AveragedPerceptron0(sentences)
# print A.tagdict
# print len(A.weights.keys())
# print A.tags
# print A.weights[A.weights.keys()[0]].items()
# A.train(verbose=True)
# tagged_sentences=A.tag_new_sentences(new_sentences)
# print tagged_sentences
# print A.test_accuracy(test_sentences)
